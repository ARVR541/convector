...
3. Хук `client/src/hooks/useRates.ts` берет курсы:
   ...
   - затем из backend `GET /api/rates`.
4. Бэкенд (`server/src/routes/rates.ts`) отдает:
   - свежий кэш из памяти, или
...
### Хуки и состояние
- `client/src/hooks/useRates.ts` — загрузка курсов, TTL-кэш, fallback, retry.
...
- `client/src/utils/constants.ts` — константы приложения (валюты, ключи, TTL).
...
- `server/src/routes/rates.ts` — `GET /api/rates`, кэш-логика, fallback на stale.
- `server/src/services/cbrService.ts` — запрос в ЦБ РФ и нормализация `rubPerUnit`.
...
- `currencyRates` — кэш курсов.
- `ratesMeta` — дата/время обновления курсов.
