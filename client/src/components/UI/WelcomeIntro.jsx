...
export const WelcomeIntro = ({ onFinish }) => {
    ...
    useEffect(() => {
        ...
        lineAppearanceDelays.forEach((delay, index) => {
            timers.push(window.setTimeout(() => {
                setVisibleLines((previous) => previous.map((value, lineIndex) => (lineIndex === index ? true : value)));
            }, delay));
        });
    ...
    return (<div className={`welcome-overlay ${isClosing ? "is-closing" : ""}`.trim()} aria-live="polite">
      <div className="welcome-content">
        ...
        <button type="button" className="welcome-skip" onClick={handleSkip} aria-label="Пропустить вступительную анимацию">
          Пропустить
        </button>
      </div>
    </div>);
