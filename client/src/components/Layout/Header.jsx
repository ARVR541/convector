...
export const Header = ({ theme, navItems, currentPath, onNavigate, onToggleTheme }) => {
    ...
    return (<header className="site-header">
      ...
      <nav aria-label="Разделы сайта" className="site-nav">
        {navItems.map((item) => {
            ...
            return (<a key={item.path} href={item.path} className={`nav-link ${isActive ? "is-active" : ""}`.trim()} aria-label={`Перейти к разделу ${item.label}`} aria-current={isActive ? "page" : undefined} onClick={(event) => handleNavClick(event, item.path)}>
              {item.label}
            </a>);
        })}
      </nav>
