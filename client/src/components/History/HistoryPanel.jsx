...
export const HistoryPanel = ({ items, onClearHistory }) => {
    ...
    return (<section className="glass-panel history-card" id="history" aria-labelledby="history-title">
      <div className="history-head">
        <h2 id="history-title">История конвертаций</h2>

        <button type="button" className="danger-button" onClick={handleClearClick} disabled={items.length === 0} aria-label="Очистить историю конвертаций">
          Очистить историю
        </button>
      </div>
      ...
      {items.length === 0 ? (<p className="history-empty">Пока нет конвертаций. Здесь появятся последние 10 успешных операций.</p>) : (
