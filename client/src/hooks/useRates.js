...
const isValidRatesBucket = (value) => {
    if (!value || typeof value !== "object") {
        return false;
    }
    return Object.values(value).every((bucketValue) => isValidRatesObject(bucketValue));
};
const isValidRatesMetaBucket = (value) => {
    if (!value || typeof value !== "object") {
        return false;
    }
    return Object.values(value).every((bucketValue) => isValidRatesMeta(bucketValue));
};
const normalizeRatesBucket = (value) => {
...
const normalizeRatesMetaBucket = (value) => {
...
const readCachedRatesState = (cacheKey) => {
    const cachedRates = readStorage(STORAGE_KEYS.currencyRates, null);
    const cachedMeta = readStorage(STORAGE_KEYS.ratesMeta, null);
...
const isCacheFresh = (meta) => Date.now() - meta.timestamp < RATES_TTL_MS;
// Единая точка записи кэша, чтобы не дублировать логику по ключам.
const persistRatesState = (cacheKey, state) => {
    const rawRates = readStorage(STORAGE_KEYS.currencyRates, {});
    const rawMeta = readStorage(STORAGE_KEYS.ratesMeta, {});
...
export const useRates = ({ notify, requestedDate = null } = {}) => {
    const [ratesState, setRatesState] = useState(null);
    ...
    const loadRates = useCallback(async (fallbackRatesState, showLoader) => {
