...
const App = () => {
    ...
    const [history, setHistory] = useLocalStorage(STORAGE_KEYS.conversionHistory, []);
    ...
    const handleRememberPair = useCallback((from, to) => {
    ...
    return (<div className="app-root" data-theme={theme}>
      ...
      <div className={`app-shell ${isIntroFinished ? "is-ready" : "is-hidden"}`.trim()} aria-hidden={!isIntroFinished}>
        <Header theme={theme} navItems={HEADER_NAV_LINKS} currentPath={currentRoute} onNavigate={navigate} onToggleTheme={toggleTheme}/>
        ...
        <main className="app-main">
          ...
          {currentRoute === "/" ? (<>
              ...
              {hasFatalError ? (<ErrorState title="Курсы недоступны" message={error ?? "Не удалось загрузить курсы валют."} onRetry={handleRetry} isRetrying={isLoading}/>) : (<SectionReveal delayMs={80}>
                  <ConverterCard ratesState={ratesState} disabled={isLoading || !ratesState} initialFrom={settings.preferredFrom} initialTo={settings.preferredTo} selectedRateDate={selectedRateDate} maxRateDate={maxRateDate} onRateDateChange={setSelectedRateDate} onRememberPair={handleRememberPair} onConverted={handleConverted} onNotify={pushToast}/>
                </SectionReveal>)}
            </>) : null}

          {/* Отдельная страница истории */}
          {currentRoute === "/history" ? (<SectionReveal delayMs={80}>
          ...
          {currentRoute === "/about" ? (<SectionReveal delayMs={80}>
