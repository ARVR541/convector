...
- `date=YYYY-MM-DD` — получить исторические курсы на указанную дату.
...
Фронтенд сохраняет:
- `currencyRates`: кэш курсов по ключам даты (`latest` и `date:YYYY-MM-DD`)
- `ratesMeta`: метаданные кэша по ключам даты (`timestamp`, `date`, `lastUpdatedLocalISO`)
